            ...
            var settings = $.extend({}, this.opts, options),
                    ...
                    for (i in settings.queryBy) {
                        if (itemProps[settings.queryBy[i]]) {
                            var item = itemProps[settings.queryBy[i]].toLowerCase(),
                                emails = (this.query.toLowerCase()).match(new RegExp(settings.delimiter + '\\w+', "g")),
                                j;
                            if ( !! emails) {
                                for (j = 0; j < emails.length; j++) {
                                    var email = (emails[j].substring(1)).toLowerCase(),
                                        re = new RegExp(settings.delimiter + item, "g"),
                                        used = ((this.query.toLowerCase()).match(re));

                                    if (item.indexOf(email) != -1 && used === null) {
                                        return true;
                                    }
                                }
                            }
                    ...
                    if (items.length && settings.sensitive) {
                        ...
                        if (currentUser.length == 1) {
                            for (i = 0; i < len; i++) {
                                ...
                                if ((currentRes.email[0] == currentUser)) {
                                    priorities.highest.push(currentRes);
                                }
                                else if ((currentRes.email[0].toLowerCase() == currentUser.toLowerCase())) {
                                    priorities.high.push(currentRes);
                                }
                                else if (currentRes.email.indexOf(currentUser) != -1) {
                ...
                _render = function(items) {
                    ...
                    items = $(items).map(function(i, item) {
                        ...
                        i = $(that.options.item).attr('data-value', item.email);
                        ...
                        if (item.email) {
                            _linkHtml.append('<span class="mention_email"> ' + settings.delimiter + item.email + '</span>');
                        }
